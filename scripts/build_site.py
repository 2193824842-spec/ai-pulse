"""
build_site.py — Reads posts/index.json and rebuilds blog.html + index.html card sections.
Run after every publish: python build_site.py
"""
import json
import os
import re
...
def update_stats(html_path, posts):
    """Update stat numbers in homepage."""
    with open(html_path, 'r', encoding='utf-8') as f:
        html = f.read()
...
def render_related_html(related_posts, lang='en', base_path='/ai-pulse/posts'):
    """Render the related articles section HTML."""
...
def inject_related_posts(posts, lang='en'):
    """Inject related articles into each post HTML file."""
    if lang == 'en':
    ...
    for post in posts:
        ...
        with open(html_path, 'r', encoding='utf-8') as f:
            html = f.read()
        # Skip if dynamic JS section is present — template handles it
        if 'id="related-section"' in html:
            continue
        # Only inject if the markers exist or we can add before </article>
...
def main():
    ...
    # Inject related articles into each post
    ...
    if os.path.exists(ZH_INDEX_JSON):
        ...
        # Build Chinese blog page tag buttons
        ...
        # Inject related articles into Chinese posts
